...
def subEnumerate(program):
	...
	for domain in f:
		...
		try:
			os.system('./links/amass enum -config ./amass_config.ini -exclude "Brute Forcing" -ef /opt/autofindomain/amass/exclue.txt -w /opt/autofindomain/amass/dns.txt -d '+domain+' -noalts -o out.txt -min-for-recursive 2 -rf ./dns_resolvers.txt')
