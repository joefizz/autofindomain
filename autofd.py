...
def subEnumerate(program):
	...
	for domain in f:
		...
		try:
			os.system('./links/amass enum -config ./amass_config.ini -nolocaldb -ef /opt/autofindomain/amass/source_exclude.txt -w /opt/autofindomain/amass/dns.txt -d '+domain+' -noalts -o out.txt -min-for-recursive 2 -rf ./dns_resolvers.txt')
	...
	for domain in f:
		...
		for subdomain in newSubdomains:
			...
			try:
				...
				proc = subprocess.call(['nmap','-n','--open','-p','80,81,300,443,591,593,832,981,1010,1090,1098,1099,1111,1311,2082,2087,2095,2096,2480,3000,3128,3333,4080,4243,4443,4444,4445,4567,4711,4712,4786,4993,5000,5104,5108,5555,5556,5800,6543,7000,7001,7002,7003,7004,7070,7071,7396,7474,8000,8001,8002,8003,8008,8014,8042,8069,8080,8081,8088,8090,8091,8118,8123,8172,8222,8243,8280,8281,8333,8443,8500,8686,8834,8880,8888,8983,9000,9001,9002,9003,9012,9043,9060,9080,9090,9091,9200,9443,9503,9800,9981,10999,11099,12443,16080,18091,18092,20720,28017,45000,45001,47001,47002,50500']+nmap_args, stdout=FNULL, stderr=subprocess.STDOUT)
			except OSError as e:
				print (e.output)
...
def main():
	...
	if (sys.argv[1]) == "slack":
		if len(sys.argv) < 3:
			...
			try:
				r = requests.post(slack_api+'chat.postMessage', json={"text":"slack message","channel":slack_channel}, headers={'Content-Type':'application/json','Authorization':'Bearer '+slack_oauth_token})
				print(r.content)
			except Exception as e:
				print(tbad,e,tend)
