...
def subEnumerate(program):
	...
	for domain in f:
		...
		try:
			os.system('./links/amass enum -config ./amass_config.ini -src -nolocaldb -ef /opt/autofindomain/amass/source_exclude.txt -exclude "Brute Forcing" -w /opt/autofindomain/amass/dns.txt -d '+domain+' -noalts -o out.txt -min-for-recursive 2 -rf ./dns_resolvers.txt')
