...
def nuclei(program, linux):
	...
	if linux == 'true':
		print('nuclei linux')
		os.system('cat ./programs/'+program+'/urls-'+timestamp+'.txt | ./nuclei/linux/nuclei -update-directory ./nuclei/nuclei-templates -silent -t technologies/ -t vulnerabilities/ -t default-credentials/ -t subdomain-takeover/ -t cves/ -t files/ -o ./programs/'+program+'/nuclei-out-'+timestamp+'.txt')
	else:
		print('nuclei mac')
		os.system('cat ./programs/'+program+'/urls-'+timestamp+'.txt | ./nuclei/mac/nuclei -update-directory ./nuclei/nuclei-templates -silent -t technologies/ -t vulnerabilities/ -t default-credentials/ -t subdomain-takeover/ -t cves/ -t files/ -o ./programs/'+program+'/nuclei-out-'+timestamp+'.txt')
